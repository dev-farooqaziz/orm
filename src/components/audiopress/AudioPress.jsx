                ...
                <div className="container">
                    <div className="grid grid-cols-12 items-center gap-y-10 md:gap-10">
                        ...
                        <div className="col-span-12 lg:col-span-6">
                            ...
                            <div className="flex items-center justify-between bg-white p-3 md:p-5 rounded-[10px] mt-5">
                                <div className="flex items-center gap-5 w-full">
                                    <Image src={audioImg} alt="ORM" className="w-[80px] md:w-[100px]" />
                                    <div className="flex flex-col gap-4">
                                        <h3 className="text-[16px] md:text-[20px] font-semibold font-poppins text-black">
                                            SpeakEasy Sessions
                                        </h3>
                                        <span className="text-[14px] md:text-[16px] tracking-wide font-poppins text-[#ababab]">Episode 1 - Season 2</span>
                                        <div className="flex items-center gap-3">
                                            <span className="text-[10px] font-poppins text-black">{formatTime(currentTime)}</span>
                                            <span className='bg-[#D9D9D9] rounded-[10px] h-[4px] w-[125px] md:w-[150px] xl:w-[300px] relative'>
                                                <span
                                                    className='bg-[#24BF5A] h-full block absolute top-0 left-0 rounded-[10px]'
                                                    style={{ width: `${progress}%` }}
                                                ></span>
                                            </span>
                                            <span className="text-[10px] font-poppins text-black">00:14</span>
                                            <audio ref={audioRef} src="/Separate-Ways.mp3" type="audio/mpeg" />
                                        </div>
                                    </div>
                                </div>
                                <div className="flex items-center justify-end gap-4 w-max">
                                    <button onClick={togglePlay} className='w-[25%] md:w-[30%]'>
                                        {isPlaying ? <Image src={pause} alt="ORM" /> : <Image src={play} alt="ORM" />}
                                    </button>
                                    <button className='w-[25%] md:w-[30%]'>
